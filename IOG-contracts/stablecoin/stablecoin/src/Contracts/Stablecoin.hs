...
import           Plutus.Contract.StateMachine (AsSMContractError,
                                               SMContractError, State (..),
                                               StateMachine,
...
-- KPLC The following imports were added to allow for the generation of
-- KPLC UPLC code for the contract's policy.
import           Plutus.Script.Utils.V1.Typed.Scripts.Validators as Validators
...
-- KPLC The following bindings were added to generate UPLC code
-- KPLC for the contract's policy 
typedValidator' :: PlutusTx.CompiledCode (Stablecoin -> Validators.UntypedValidator)
typedValidator' =
    let validator = Validators.mkUntypedValidator . SM.mkValidator . stablecoinStateMachine
    in $$(PlutusTx.compile [|| validator ||])

unDeBruijnProgram
     :: Program NamedDeBruijn uni fun ann -> QuoteT (Either FreeVariableError) (Program Name uni fun ann)
unDeBruijnProgram (Program ann ver term) = Program ann ver Haskell.<$> unDeBruijnTerm term
